	...
	},
	{
		...
		choices = {
			{ text = "Yes", effect = function() 
				...
				print("Scavenged spaceship and found " .. foundFuel .. " fuel, " .. foundOxygen .. " oxygen, and " .. foundMoney .. " money.")
			end },
			{ text = "No", effect = function() print("Ignored derelict spaceship") end },
		},
		image = "planet.png",
	},
	{
		...
		choices = {
			...
			{ text = "No", effect = function() print("Ignored weak signal") end },
		},
		image = "planet.png",
	},
	-- planet
	{
		...
		choices = {
			{ text = "Yes", effect = function() 
				...
				Resources.oxygen = Resources.oxygen + foundOxygen
				Resources.money = Resources.money + foundMoney
				print("Explored planet and found " .. foundFuel .. " fuel, " .. foundOxygen .. " oxygen, and " .. foundMoney .. " money.")
			end },
			{ text = "No", effect = function() print("Ignored planet") end },
		},
		image = "planet.png",
	},
}
...
function love.load()
	...
	function layoutmanager:draw()
		...
		love.graphics.printf("New Game", love.graphics.getWidth() / 2 - 75, love.graphics.getHeight() / 2 - 10, 150, "center")
		love.graphics.printf("Continue", love.graphics.getWidth() / 2 - 75, love.graphics.getHeight() / 2 + 100, 150, "center")
	end
	function layoutmanager:update(dt)
		if love.mouse.isDown(1) then
			...
			if mx >= love.graphics.getWidth() / 2 - 50 and mx <= love.graphics.getWidth() / 2 + 50 and
			   my >= love.graphics.getHeight() / 2 + 100 and my <= love.graphics.getHeight() / 2 + 150 then
				if Menu.navController and Menu.navController.navigateTo then
					Menu.navController:navigateTo(types.GameStateType.Gameplay)
				else print("NavController or navigateTo function not defined")
				end
...
function love.update(dt)
	...
	elseif GameState == types.GameStateType.Gameplay then
		...
		if Moving then
			...
			PrevPlanetPosition.x = PrevPlanetPosition.x + (Direction.x * love.graphics.getWidth() - PrevPlanetPosition.x) * dt
			PrevPlanetPosition.y = PrevPlanetPosition.y + (Direction.y * love.graphics.getHeight() - PrevPlanetPosition.y) * dt
			NewPlanetPosition.x = NewPlanetPosition.x + (Direction.x * love.graphics.getWidth() - NewPlanetPosition.x) * dt
			NewPlanetPosition.y = NewPlanetPosition.y + (Direction.y * love.graphics.getHeight() - NewPlanetPosition.y) * dt
			...
			for i, choice in ipairs(CurrentNode.choices) do
				if mx >= love.graphics.getWidth() / 2 - 200 + (i-1)*250 and mx <= love.graphics.getWidth() / 2 - 200 + (i-1)*250 + 200 and
				   my >= love.graphics.getHeight() / 2 + PLANET_RADIUS + 60 and my <= love.graphics.getHeight() / 2 + PLANET_RADIUS + 90 then 
					...
					insert(PreviouslyVisitedCoords, { x = PlayerPosition.x, y = PlayerPosition.y })
				end
			end
		end
		...
		if CurrentNode and not visited then
			...
			if charIndex < #CurrentNode.question and timer >= typingSpeed then
					...
					print(displayedText)
					-- reset charIndex and displayedText when done
			end
...
function handleNavigateToNewNode(direction)
	...
	Moving = true
	Direction = direction
	NewPlanetPosition = { x = -direction.x * love.graphics.getWidth()*3, y = -direction.y * love.graphics.getHeight()*3 }
...
function love.draw()
	...
	elseif GameState == types.GameStateType.Gameplay then
		...
		if Moving then
			...
			love.graphics.draw(prevImg, love.graphics.getWidth() / 2 + PrevPlanetPosition.x, love.graphics.getHeight() / 2 + PrevPlanetPosition.y, time % (math.pi * 2), (PLANET_RADIUS * 2) / prevImg:getWidth(), (PLANET_RADIUS * 2) / prevImg:getHeight(), prevImg:getWidth() / 2, prevImg:getHeight() / 2)
			love.graphics.draw(newImg, love.graphics.getWidth() / 2 + NewPlanetPosition.x, love.graphics.getHeight() / 2 + NewPlanetPosition.y, time % (math.pi * 2), (PLANET_RADIUS * 2) / newImg:getWidth(), (PLANET_RADIUS * 2) / newImg:getHeight(), newImg:getWidth() / 2, newImg:getHeight() / 2)
		elseif CurrentNode then
			local curImg = love.graphics.newImage(CurrentNode.image)
			love.graphics.draw(curImg, love.graphics.getWidth() / 2, love.graphics.getHeight() / 2, time % (math.pi * 2), (PLANET_RADIUS * 2) / curImg:getWidth(), (PLANET_RADIUS * 2) / curImg:getHeight(), curImg:getWidth() / 2, curImg:getHeight() / 2)
		end
		...
		if not Moving and not visited then
			-- print alien in bottom left corner
			local alien = love.graphics.newImage("alien.png")
			love.graphics.draw(alien, 10, love.graphics.getHeight() - 110, 0, 100 / alien:getWidth(), 100 / alien:getHeight())
			
			-- print question text
			love.graphics.printf(displayedText, 0, love.graphics.getHeight() / 2 + PLANET_RADIUS + 20, love.graphics.getWidth(), "center")
			for i, choice in ipairs(CurrentNode.choices) do
